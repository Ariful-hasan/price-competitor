...
class PriceSyncService
{
    ...
    public function syncAllPrices(array $fetchUrls, Carbon $fetchAt): void
    {
        foreach ($fetchUrls as $key) {
            ...
            $this->productLowestPriceRepository->saveLowestPrice(
                (int) $key[static::PRODUCT_ID], $prices['vendor_name'], $prices['price'], $fetchAt
            );
        }
    }
     ...
     * @return array
     */
    private function fetchAndCalculatePrices(array $config): array
    {
     ...
     */
    private function processStream(mixed $stream, ExtractProductPriceContract $extractor, array $config): array
    {
        ...
        foreach ($items as $item) {
            $data = $extractor->extractProduct((array)$item);
            $prices = $this->calculateProductLowestPrice((int)$config[static::PRODUCT_ID], $data);
        }
     ...
     * @return array
     */
    private function calculateProductLowestPrice(int $productId, array $item): array
    {
        $data = Cache::get(static::CACHE_KEY . $productId);

        if (!$data) {
            Cache::set(static::CACHE_KEY . $productId, $item);
